...
class Work(object):
    ...
    def domain(self):
        ...
        # 100根
        url1 = 'https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=1h&limit=100'
        flag = 0
        open_time = ""
        duocang = 0
        kongcang = 0
        ...
        while 1:
            ...
            for value in data1:
                List_data_checked1.append(float(value[5]))
            # 计算出均值
            ...
            # 100根的平均值
            mean_checked1 = np.mean(List_data_checked1)
            ...
            if flag == 0 and open_time != str(self.timeStamp(data[-1][0])):
                if float(data[-1][5]) > mean_checked and float(data[-1][5]) > mean_checked1:
                    # 最新价大于开盘价 开多
                    if price_last > float(data[-1][1]):
                        price_max = price_last
                        open_price = price_last
                        print('开多')
                        temp = {}
                        temp['open'] = '开多'
                        ...
                        temp['10根成交量平均值'] = mean_checked
                        temp['100根成交量平均值'] = mean_checked1
                        # 记录开盘时间
                ...
                # 条件1平多仓
                if duocang == 1:
                    if price_last - open_price > 1 / 1000 * price_last and profit1 == 0:
                        profit1 = price_last
                    elif profit1 != 0 and price_last - profit1 < -2 / 1000 * price_last:
                        print("第1条件平多仓，止盈千1，回调千2，亏掉千1")
                        duocang = 0
                        flag = 0
                        open_temp = "平多"
                        temp = {}
                        ...
                        temp['最高点'] = price_max
                        temp['千1止盈价'] = profit1
                        temp['平仓条件'] = "第1条件平多仓/止盈千1/回调千2/亏掉千1"
                        profit1 = 0
                        profit2 = 0
                        # 保存数据
                        t1 = threading.Thread(target=save_data, args=(temp, json_name))
                ...
                # 条件2平多仓
                if duocang == 1:
                    if price_last - open_price > 2 / 1000 * price_last and profit2 == 0:
                        profit2 = price_last
                    elif profit2 != 0 and price_last - profit2 < -1 / 1000 * price_last:
                        print("第2条件平多仓，止盈千2，回调千1，利润千1")
                        duocang = 0
                        flag = 0
                        open_temp = "平多"
                        temp = {}
                        ...
                        temp['最高点'] = price_max
                        temp['千2止盈价'] = profit2
                        temp['平仓条件'] = "第2条件平多仓/止盈千2/回调千1/利润千1"
                        profit1 = 0
                        profit2 = 0
                        # 保存数据
                        t1 = threading.Thread(target=save_data, args=(temp, json_name))
                ...
                # 条件1平空仓
                if kongcang == 1:
                    if price_last - open_price < -1 / 1000 * price_last and profit3 == 0:
                        profit3 = price_last
                    elif profit3 != 0 and price_last - profit3 > 2 / 1000 * price_last:
                        print("第1条件平空仓，止盈千1，回调千2，亏掉千1")
                        kongcang = 0
                        flag = 0
                        open_temp = "平空"
                        temp = {}
                        ...
                        temp['最低点'] = price_min
                        temp['千1止盈价'] = profit3
                        temp['平仓条件'] = "第1条件平空仓/止盈千1/回调千2/亏掉千1"
                        profit3 = 0
                        profit4 = 0
                        # 保存数据
                        t1 = threading.Thread(target=save_data, args=(temp, json_name))
                ...
                # 条件2平空仓
                if kongcang == 1:
                    if price_last - open_price < - 2 / 1000 * price_last and profit4 == 0:
                        profit4 = price_last
                    elif profit4 != 0 and price_last - profit4 > 1 / 1000 * price_last:
                        print("第2条件平空仓，止盈千2，回调千1，利润千1")
                        kongcang = 0
                        flag = 0
                        open_temp = "平空"
                        temp = {}
                        ...
                        temp['最低点'] = price_min
                        temp['千2止盈价'] = profit4
                        temp['平仓条件'] = "第2条件平空仓/止盈千2/回调千1/利润千1"
                        profit3 = 0
                        profit4 = 0
                        # 保存数据
                        t1 = threading.Thread(target=save_data, args=(temp, json_name))
