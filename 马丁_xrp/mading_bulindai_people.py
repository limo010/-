...
json_name = 'set_people.json'
json_name1 = 'PEOPLE日志.json'
symbol = "PEOPLE_USDT"
# 程序每一次开启都要读取这个风险仓位###################################################
try:
...
def trade():
    ...
    try:
        ...
        # 不同交易所开单方式不同
        quantity = int(round(float(data['contracts']) / 100, 0))
    ...
    while 1:
        try:
            ...
            if len(risk_positions) != 0 and work.symbol_price_last != 0:
                print("触发警戒线===================================================================================>")
                print('首次开仓价格', price1, '首次开仓仓位', position1, '最新价', work.symbol_price_last, '未处理的风险仓位', risk_positions, '已处理的风险仓位', hangding_risk_positons, '待补仓的仓位', risk_positions_,'下轨',lower_band[0], 'K线开盘的时间', K_time)
            ...
            if len(risk_positions) == 0 and len(hangding_risk_positons) == 0:
                print("当前没有风险!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                print('首次开仓价格', price1, '首次开仓仓位', position1, '最新价', work.symbol_price_last, '未处理的风险仓位', risk_positions, '已处理的风险仓位', hangding_risk_positons, 'K线开盘的时间', K_time, '待补仓的仓位', risk_positions_,'下轨', int(lower_band[0]))
        except Exception as e:
            print("处理风险出现问题", e)
...
if __name__ == '__main__':
    # 接收K线数据
    work_one = threading.Thread(target=get_K_data,args=('PEOPLEUSDT', '15m'))
    # 接收最新价格
    work2 = Bn_data_wss("PEOPLE")
    ...
    # work_two.join()
    # work2_one.join()
    app.run(host='127.0.0.1', port=7792, debug=False)
