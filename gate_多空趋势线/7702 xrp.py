...
if __name__ == '__main__':
    # 获取全局变量
    json_name1_ = 'xrp_7702_多空趋势线日志.json'
    error_ = 'error_7702_日志.json'
    json_name2_ = 'set_7702.json'
    key_ = ''
    secret_ = ''
    precision = 0
    ...
    time.sleep(3)
    # 接收K线开盘时间
    thread4 = threading.Thread(target=bs.get_K_data, args=("XRPUSDT", "2h"))
    thread4.start()
    time.sleep(3)
    # 盈利回实际亏损
    ...
    thread3.start()
    # 止损平仓时计算出实际亏损或者是实际盈利
    thread5 = threading.Thread(target=bs.stop_loss, args=("XRP_USDT", work))
    thread5.start()
    time.sleep(3)
    # 仓位千4 止盈
    thread6 = threading.Thread(target=bs.take_profit, args=("XRP_USDT", work))
    thread6.start()
    # 判断是否波动5%
